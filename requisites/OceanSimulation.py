...
class OceanWidget(QMainWindow, OceanWindow.Ui_MainWindow):
    ...
    def HereAreWeGo(self):
        ...
        print(VT)
        T = (np.sqrt(-(Y-YT)**2*Vx**2+2*Vy*(Y-YT)*(X-XT)*Vx-(X-XT)**2*Vy**2+VT**2*(X**2-2*X*XT+XT**2+(Y-YT)**2))+(X-XT)*Vx+(Y-YT)*Vy)/(-Vy**2-Vx**2+VT**2)
        print(T)
        PhiT = np.arctan2((Y + Vy * T - YT) / (VT * T), (X + Vx * T - XT) / (VT * T))
        print(PhiT)
